...
} from "../renderers/github-actions.js";
...
const supportedRenderers = [
  createAgentsMdRenderer(),
  createCursorRenderer(),
  createDependencyCruiserRenderer(),
  createGitHubActionsRenderer(),
] satisfies Renderer[];
...
function createDependencyCruiserRenderer(): Renderer {
  return {
    outputPath: DEPENDENCY_CRUISER_TARGET_FILE,
    render(model) {
      return renderDependencyCruiserConfig(model);
    },
    target: "dependencyCruiser",
  };
}

function createGitHubActionsRenderer(): Renderer {
  return {
    outputPath: GITHUB_ACTIONS_WORKFLOW_PATH,
    render(model) {
      return renderGitHubActionsWorkflow(model).contents;
    },
