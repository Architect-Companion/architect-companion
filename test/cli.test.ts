...
describe("runCli", () => {
  ...
  it("renders selected targets for a valid project", async () => {
    const { io, writes } = createIo();
    ...
    try {
      ...
      expect(exitCode).toBe(0);
      expect(writes.stdout).toBe("created AGENTS.md\n");
      expect(writes.stderr).toBe("");
  ...
  it("fails render --check when generated targets are stale", async () => {
    const { io, writes } = createIo();
    const projectDir = copySampleProject();
    const profilesDir = fileURLToPath(new URL("../profiles", import.meta.url));
    ...
    try {
      ...
      expect(exitCode).toBe(1);
      expect(writes.stdout).toBe("");
      expect(writes.stderr).toBe("AGENTS.md is stale (missing).\n");
