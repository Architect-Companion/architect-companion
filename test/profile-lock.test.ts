...
describe("resolveProfileLockStatus", () => {
  it("returns missing when the lock file does not exist", async () => {
    ...
    try {
      ...
      expect(
        status.expected.profiles.map((profile) => `${profile.name}@${profile.version}`),
      ).toEqual(["modular-monolith@0.2.0", "typescript@0.1.0"]);
      expect(status.expected.profiles[0]?.contentHash).toMatch(/^sha256-[0-9a-f]{64}$/);
    } finally {
      rmSync(projectDir, { force: true, recursive: true });
    }
  });

  it("returns match when the lock matches the resolved profiles", async () => {
    const projectDir = copySampleProject();
    try {
      const model = await loadEffectiveHarnessModel({ profilesDir, projectDir });
  ...
  it("returns stale with different-content-hash when the profile content drifted", async () => {
    ...
    try {
      ...
      await writeProfileLock(projectDir, {
        ...initial.expected,
        profiles: initial.expected.profiles.map((profile, index) =>
          index === 0
            ? {
                ...profile,
                contentHash:
  ...
  it("rounds trips through formatProfileLock", async () => {
    ...
    try {
      ...
      expect(serialized).toContain(status.expected.profiles[0]?.contentHash);
...
function defaultLock(): {
  profiles: Array<{ contentHash: string; name: string; version: string }>;
  schemaVersion: 1;
} {
  return {
    profiles: [
